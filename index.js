...
function generateRandomPassword(length) {
  ...
  let randomCharacters = characters.charAt(randomIndex)
...
function roundObjectValues(obj) {
  ...
  let rounded = arrObj.map(([key, value]) => {
  ...
  let arrResult = Object.fromEntries(rounded)
...
function estimateInvestment(principal, interestRate, years) {
  ...
  if (!typeof principal === "number" && !typeof interestRate === "numver" && typeof years === "number") {
// Повертаємо null, що свідчить про неможливість обробки вхідних даних.
return null
  }  
...
function isTotalPriceExceedsMaxPrice(products, maxPrice) {
  ...
  let totalPrice = products.reduce((acum, amount) => {
    acum + amount.price       
  })  
